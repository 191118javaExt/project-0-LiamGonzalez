...
public class EmployeeAdminLogic {
	...
	AccountsProcess acctProc = new AccountsProcess();
	...
	AccountsDAO bDAO = new AccountsDAOImpl();
	...
	public void setCustomerApproval(Customer customer) {
		System.out.println("Current Approval Status: " + customer.Approved());
	...
	public void existingAdminMainMenu(Employee employee) {
		...
		if (cDAO.customerExists(custUN)) {
			...
			System.out.println("Make a selection:");
				...
				if (typeOfTrans == 5) {
					Account whichAcct = Bank.existingCustomerAccountsDisplay(currentCust);
				...
				if (typeOfTrans == 3) {
					...
					do {
						...
						acctProc.doTransaction(whichAcct, trans);
					} while (trans != 4);
				}
				if (typeOfTrans == 2) {
					setCustomerApproval(currentCust);
